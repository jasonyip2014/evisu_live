<?php
class  Creare_Latesttweet_Model_Tweets
{
    private $tweets = null;
    private $tweetNumber = 0;
    const TWEETS_ON_PAGE_COUNT = 4;
    ...
    public function getTweets($tweets)
    {
        ...
        foreach($tweets as $tweet)
        {
            ...
            $t[$i]["date"] = trim($tweet->created_at);
            $i++;
        }
        return $t;
    }
    ...
    public function getLatestTweets()
    {
        if(is_null($this->tweets))
        {
            ...
            if($latesttweets){
                $this->tweets = $this->getTweets($latesttweets);
            }
        }
        return $this->tweets;
    }
    ...
    {
        $number = $this->tweetNumber;

        $page = Mage::app()->getRequest()->getParam('page');
        if($page)
        {
            $number += ($page-1) * self::TWEETS_ON_PAGE_COUNT;
        }

        $number %= $this->getNumberOfTweets();
        $tweets = $this->getLatestTweets();
        $this->tweetNumber++;
        if (isset($tweets[$number]))
        {
            return array(
                'date' => date('d F', strtotime($tweets[$number]['date'])),
                'text' => $tweets[$number]['tweet'],
            );
        }
        return null;
    }
    public function getCurrentTweetNumber()
    {
