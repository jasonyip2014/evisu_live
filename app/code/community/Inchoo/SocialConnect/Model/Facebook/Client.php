...
class Inchoo_SocialConnect_Model_Facebook_Client
{
    ...
    protected function fetchAccessToken()
    {
        ...
        /////// fix for String and ClassStd $reaponse result correct working
            $token = null;
            ...
            else
            {
                ...
                {
                    $response_param = explode('&', $response);
                    $response_token = explode('=', $response_param[0], 2);
                    $token = $response_token[1];
                }
            }
            $this->token = $token;
            ...
            if(!$this->token)
            {
                Mage::getSingleton('core/session')
                    ->addNotice(
                        $this->__('Facebook return bad response format.')
                    );
            }
        ///////!fix
    }
