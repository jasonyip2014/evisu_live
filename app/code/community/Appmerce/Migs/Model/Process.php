...
class Appmerce_Migs_Model_Process extends Varien_Object
{
    /**
    ...
    {
        return Mage::getSingleton('migs/api');
    }
    ...
    public function invoice($order)
    {
        ...
        if (!$order->hasInvoices() && $order->canInvoice()) {
            ...
            if ($invoice->getTotalQty() > 0) {
                ...
                $transaction->save();
                $invoice->addComment(Mage::helper('migs')->__('Automatic invoice.'), false);
    ...
    {
        $this->restore();
        $message = Mage::helper('migs')->__('Payment failed. Please try again.');
