...
class Ikantam_InstagramConnect_Helper_Image extends Mage_Core_Helper_Abstract
{
    ...
    protected function getEndPointUserUrl()
    {
        return 'https://api.instagram.com/v1/users/'.$this->getUserId().'/media/recent/?client_id='.$this->getClientId().'&count='.$this->getImagesCount().'#';
    }
    ...
    {
        return Mage::getStoreConfig('ikantam_instagramconnect/module_options/user_id');
    }

    protected function getImagesCount()
    {
        return Mage::getStoreConfig('ikantam_instagramconnect/module_options/blog_limit');
    }
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->getEndPointUserUrl());
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_HEADER, 0);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
        ...
        if (!isset($data->meta, $data->meta->code) || $data->meta->code !== 200) {
            unset($data);
            //var_dump($this->__("Instagram connect error"));
            return null;
        }

        if (!isset($data->data)) {
            //var_dump($this->__("Instagram data not founded"));
            return null;
        }
        ...
        foreach ($data->data as $item)
        {
            ...
            $image->setStandardResolutionUrl($item->images->standard_resolution->url)
                ->setLowResolutionUrl($item->images->low_resolution->url)
                ->setThumbnailUrl($item->images->thumbnail->url)
                ->setImageId($item->id)
                ->setUsername($item->user->username)
                ->setCaptionText(($item->caption)?$item->caption->text:'');
            $images[] = $image;
        }
        return $images;
    }
