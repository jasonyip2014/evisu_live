...
class Mage_Adminhtml_Block_Store_Switcher extends Mage_Adminhtml_Block_Template
{
    ...
    const SUPER_ADMIN_ROLE = 'Administrators';
    /**
     * @var array
     */
    protected $_storeIds;
    ...
    {
        $storeId = $this->getRequest()->getParam($this->_storeVarName);
        /*if(!$storeId && $this->getUserRole() != self::SUPER_ADMIN_ROLE)
        {
            $storeId = $this->getDefaultStoreId();
        } Recover this unknown change by Cleargo*/
        return $storeId;
    }

    private function getDefaultStoreId()
    {
        return current(current(current($this->getWebsites())->getGroups())->getStores())->getId();
    }
    ...
    {
        return Mage::getSingleton('admin/session')->getUser()->getRole()->getRoleName();
    }
}
