...
class M1_Bridge_Action_Savefile
{
  ...
  function _saveFile($source, $destination, $width, $height, $local = '')
  {
    if (trim($local) != '') {
      ...
      if ($this->_copyLocal($local, $destination, $width, $height) == 'OK') {
        return 'OK';
      }
    }
    ...
    if ($width != 0 && $height != 0) {
      $result = $this->_scaled2($destination, $width, $height);
      if($result != 'OK') {
        return $result;
      }
    }
  ...
  function _scaled($destination, $width, $height)
  {
    ...
    return $this->_saveImage($new_image, $destination, $this->_imageType, 100);
  }

  //scaled2 method optimized for prestashop
...
class M1_Bridge_Action_Testsavefile{
  ...
  function perform($bridge)
  {
    ...
    if(is_dir($testDir)) {
      $objects = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($testDir), RecursiveIteratorIterator::SELF_FIRST);
  ...
  {
    $matches = null;
    $objects = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($this->productImagesDir), RecursiveIteratorIterator::SELF_FIRST);
    foreach($objects as $name => $object){
      if(preg_match('/\.(jpe?g|png|gif)$/',$name)) {
        return $name;
      }
